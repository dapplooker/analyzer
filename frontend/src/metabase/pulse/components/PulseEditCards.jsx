  ...
  getNotices(card, cardPreview, index) {
    ...
    if (cardPreview) {
      ...
      if (cardPreview.pulse_card_type == null && !hasAttachment) {
        notices.push({
          type: "warning",
          head: t`Heads up`,
          body: t`Raw data chart can only be included as email attachments`,
        });
      }
    }
    if (showSoftLimitWarning) {
      notices.push({
        ...
        body: t`We recommend keeping Reports small and focused to help keep them digestible and useful to the whole team.`,
      });
    }
    return notices;
  }
    ...
    return (
      <div className="py1">
        ...
        <p className="mt1 h4 text-bold text-medium">
          {t`Choose chart you'd like to send in this report`}.
        </p>
        <ol className="my3">
